class Zoom {
  ...
  calcSnap() {
    ...
    this.status.snapRight =
      Math.abs(
        this.status.imgWidth + this.status.blankLeft + this.status.blankRight - this.status.x - window.innerWidth
      ) < 5;
    this.status.snapBottom =
      Math.abs(
        this.status.imgHeight + this.status.blankBottom + this.status.blankTop - this.status.y - window.innerHeight
      ) < 5;
    if (!this.settings.showSnapIndicators) {
      return;
    }
  ...
  smoothZomm(to, mouseAt, callback, ...args) {
    ...
    for (let i = 1; i <= level; i++) {
      setTimeout(() => {
        ...
        if (this.pointZoom(from + delta * i, mouseAt) && callback) {
          callback(...args);
        }
      }, interval * i);
    }
  ...
  rectZoom(start, end) {
    ...
    if (this.settings.zoomUpperLimit && status.zoom > maxZoom) {
      status.zoom = maxZoom;
    }
    if (status.zoom < minZoom) {
      status.zoom = minZoom;
    }
  ...
  pointZoom(zoom, point) {
    ...
    if (this.settings.zoomUpperLimit && zoom > maxZoom) {
      zoom = maxZoom;
    }
    if (zoom < minZoom + 0.1) {
